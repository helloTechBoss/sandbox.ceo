...
export default function NewArticlePage() {
  ...
  return (
    <div style={{ padding: 32, maxWidth: 860 }}>
      ...
      <h1 style={{ fontFamily: "'Montserrat',sans-serif", fontWeight: 700, fontSize: '1.4rem', color: '#0F2557', marginBottom: 24 }}>New Article</h1>
      <form action={createArticle}>
        <ArticleFields article={null} />
...
function ArticleFields({ article }: { article: Record<string, unknown> | null }) {
  const t = (field: string, locale: string) => article ? ((article[field] as Record<string, string>)?.[locale] ?? '') : '';
  const v = (field: string) => article ? (article[field] as string ?? '') : '';
  ...
  return (
    <>
      <div style={{ marginBottom: 16 }}>
        ...
        <input name="slug" required defaultValue={v('slug')} style={inputStyle} />
      </div>
      ...
      <div style={{ marginBottom: 16 }}>
        ...
        <input name="categoryColor" defaultValue={v('categoryColor') || '#0F2557'} style={inputStyle} />
      </div>
      ...
      <div style={{ marginBottom: 16 }}>
        ...
        <input name="ctaWhatsappMessage" defaultValue={v('ctaWhatsappMessage')} style={inputStyle} />
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 16, marginBottom: 16 }}>
        <div>
          <label style={labelStyle}>Order</label>
          <input name="order" type="number" defaultValue={article ? (article['order'] as number ?? 0) : 0} style={inputStyle} />
        </div>
        <div>
          <label style={labelStyle}>Published At</label>
          <input name="publishedAt" type="date" defaultValue={article?.['publishedAt'] ? new Date(article['publishedAt'] as string).toISOString().slice(0, 10) : ''} style={inputStyle} />
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, paddingTop: 28 }}>
          <input name="published" type="checkbox" id="published" defaultChecked={article ? !!(article['published']) : false} style={{ width: 16, height: 16 }} />
